...
class Command(BaseCommand):
    ...
    help = (
        "Add an auth.Group that gives basic CMS Permissions. "
        "User --plugin or --cms to add additional perms."
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default=False,
            help=(
                "Superuser CMS permissions "
                "(add own users, page permissions, etc, etc)."
            ),
        )
        parser.add_argument(
            "--plugin",
            ...
            default=False,
            help=(
                "Searches for permissions with 'plugin' in their content "
                "type's app_label (ex.: cmsplugin_googlemaps), adds to group."
            ),
        )
        parser.add_argument(
            "--cms",
            ...
            required=False,
            help=(
                "Searches for permissions with 'cms' in their content "
                "type's app_label (ex.: djangocms_text_ckeditor), adds to group."
            ),
        )
        parser.add_argument(
            "--name",
...
Force overwrite of existing group and it's permissions.""",
    ...
    def handle(self, *args, **options):
        ...
        if groups.count():
            ...
            if groups.count() > 1:
                exit(
                    "more than one group with name that name "
                    "({}) found! exiting.".format(group_name)
                )
            group = groups.first()
        else:
        ...
        self.stdout.write("Following permissions were granted:")
        for perm in group.permissions.all():
            self.stdout.write("{}".format(perm))
