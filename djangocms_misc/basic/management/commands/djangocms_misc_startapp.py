...
class Command(BaseCommand):
    ...
    help = (
        "Bootstrap a new cms based app, "
        "providing app hooks and menus extension templates"
    )
...
""",
    ...
    def handle(self, *args, **options):
        ...
        if os.path.exists(target_folder):
            logger.log(
                logging.INFO,
                "Aborting, target folder already existing: %s" % target_folder,
            )
            return
        ...
        # rename
        base_templates_folder = os.path.join(
            target_folder,
            "templates",
        )
        templates_folder = os.path.join(base_templates_folder, app_name.lower())
        os.rename(os.path.join(base_templates_folder, "app"), templates_folder)
        os.rename(
        ...
        logger.log(
            logging.INFO, "- run ./manage.py test (basic concepts like class names "
        )
