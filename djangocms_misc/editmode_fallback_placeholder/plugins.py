...
def move_plugin(self, request):
    ...
    self._get_plugin_from_id(plugin_id)
    # print self
...
def assign_plugins(request, placeholders, template, lang=None, is_fallback=False):
    ...
    # If no plugin is present in the current placeholder we loop in the
    # fallback languages and get the first available set of plugins
    ...
    if not is_fallback:
        disjoint_placeholders = (
            ph for ph in placeholders if all(ph.pk != p.placeholder_id for p in plugins)
        )
        for placeholder in disjoint_placeholders:
            language_fallback = get_placeholder_conf(
                "language_fallback", placeholder.slot, template, True
            )
            editmode_language_fallback = get_placeholder_conf(
                "editmode_language_fallback", placeholder.slot, template, False
            )
            # monkey patch: check if we should display fallbacks in edit mode!
            if language_fallback and (
                not is_edit_mode(request.toolbar) or editmode_language_fallback
            ):
    ...
    # If no plugin is present in non fallback placeholders,
